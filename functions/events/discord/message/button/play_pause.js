...
if (status.paused) {
  ...
  components[0].components[0].label = "توقف مؤقت"
  ...
  components[0].components[0].label = "تشغيل"
}
...
await reply(guild_id, { content: `**${member.user.username}** ${status.paused ? 'استأنف' : 'متوقف مؤقتا'} الأغنية الحالية.`})
